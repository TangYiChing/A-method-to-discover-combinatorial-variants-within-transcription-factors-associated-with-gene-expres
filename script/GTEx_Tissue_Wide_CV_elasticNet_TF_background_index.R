...
TW_CV_model <- function(expression_RDS, geno_file, gene_annot_RDS, snp_annot_RDS, tf_annot_RDS, n_k_folds, alpha, out_dir, tis, chrom, snpset, window, tf_geno_file_dir, tf_snp_annot_dir, seed = NA) {
  ...
  #expression <- expression[, intersect(colnames(expression), rownames(gene_annot)), drop=FALSE]
  ...
  for (i in 1:length(exp_genes)) {
    ...
    if( ncol(cisgenos)>0 & n_tf>0 ){
        ...
        num_transsnps <- 0
        message("random select TFs=", list(bg.used.tf_info$TF_name))
        for( j in 1:n_tf){  # loop through each tfs for the same gene
            fname_tf_geno=Sys.glob(file.path(tf_geno_file_dir, paste0(bg.used.tf_info[j,]$TF_name, '.', 'chr', bg.used.tf_info[j,]$TF_chr, '.', '*')))
            fname_tf_snp_annot=Sys.glob(file.path(tf_snp_annot_dir, paste0('*', 'chr', bg.used.tf_info[j,]$TF_chr, '.', '*')))

            #if(!file.exists(fname_tf_geno)){
            #    next
            #}else{
            if(isTRUE(file.exists(fname_tf_geno))){ 
                used_tf = paste(bg.used.tf_info$genetfpair, sep=" ", collapse=",")
                ...
                #tf_start <- as.numeric(tf_info[j,]$TF_start) #- window  limited search region to tf boundary
                ...
                message(j, "/", n_tf, "TF ", bg.used.tf_info[j,]$TF_name, "num_tf_snps=", ncol(transgenos))
                #message(fname_tf_geno, '\n', fname_tf_snp_annot)
            
            }else{
                message(j, "/", n_tf, "TF ", bg.used.tf_info[j,]$TF_name, "num_tf_snps=0") # has TFs but NO snps
