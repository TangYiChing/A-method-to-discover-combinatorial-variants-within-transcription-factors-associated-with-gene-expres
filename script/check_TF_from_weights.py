"""
Search for trans-SNP and corresponding TF 
"""
...
if __name__ == '__main__':
    ...
    for gene in geneList:
        ...
        #print( 'gene={:} at chr={:}, \n{:}'.format(gene, geneChr, geneTransWeight) )
        # skip gene if it does not have transSNP
        if geneTransWeight.shape[0] != 0:
            gene_transWeight.update({gene:geneTransWeight})
        else:
            n_gene_wo_transSNP += 1
            #print( 'gene={:} does not have trans-SNP'.format(gene) )
    print( 'total genes={:}, found={:}, #genes has NO trans-SNP={:}'.format(len(geneList), n_found,  n_gene_wo_transSNP) )
    #print(gene_transWeight)
    ...
    # concatenate results
    if len(TFdfs) != 0:
         transSNP = pd.concat(TFdfs, axis=1) 
         print( 'transSNPs={:}\n{:}'.format(transSNP.T.shape[0], transSNP.T) )
         # save to file
         transSNP.T.to_csv(prefix+'.transSNP.txt', header=True, index=False, sep="\t")
