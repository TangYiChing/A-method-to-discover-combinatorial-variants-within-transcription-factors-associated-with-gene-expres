...
if (method == 1){
    ...
    # Save to txt
    write.table(expression, file=outFile, sep="\t", quote=F, col.names = NA, row.names = TRUE)
    ...
    #expression <- read.table(expressionfile, stringsAsFactors = FALSE, header = TRUE, sep="\t")#, row.names = TRUE)
