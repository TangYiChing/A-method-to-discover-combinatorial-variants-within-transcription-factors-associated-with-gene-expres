...
GTEXV6=paste(SPATH, "GTEx_Tissue_Wide_CV_elasticNet_TF_background_index.R", sep="")
source(GTEXV6)
# file path (change this to your own path)
expression_RDS=paste(FPATH, "expression/", PREFIX, ".expression.RDS", sep="")
gene_annot_RDS=paste(FPATH, "annotation/gene_annot/", PREFIX, ".", MODEL,".outliers.gene_annot.RDS", sep="")
...
for (run in 1:RUN){
  ...
  genotype_txt=paste(FPATH,'genotype/', PREFIX,'.chr',N_CHR,'.snp.txt', sep='')
  snp_annot_RDS=paste(FPATH, 'annotation/snp_annot/', PREFIX,'.chr',N_CHR,'.snp_annot.RDS', sep='')
